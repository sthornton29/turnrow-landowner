...
- easements_geo(id, property_id nullable, name, easement_type (powerline, pipeline, waterline_sewer, telecom_fiber, access_row, public_road_row, railroad, drainage, flowage, conservation, cemetery_access, construction_temp, solar_wind, other), relationship 'burdens_this_property'|'benefits_this_property', holder, recorded_ref, expiration_date, width_ft, elevation_ft, program, restrictions, emergency_phone, notes, acres (polygons), length_feet, miles (lines))
- assets_geo(id, property_id nullable, asset_type (well, irrigation_pivot, underground_pipe, riser, shop, shed, barn, grain_bin, grain_bin_site, house, fence, pond_dam, other), name, year_installed, condition, estimated_value, notes, details jsonb (grain_bin capacity_bu = bushels), parent_asset_id (a bin's grain_bin_site, a pivot's well), is_active, geom_geojson)
...
- farm_field_data(id, farm_connection_id, remote_field_id, crop_year, crop, planted_acres, irrigated_acres, dryland_acres, planting_date, varieties jsonb, harvested_acres, harvest_status 'complete'|'in_progress'|'unharvested' (null on pre-status rows; a yield is ACTUAL only when complete, production/harvested_acres; never compute a yield mid-harvest), production_units (null when yields are not shared), production_unit 'bu'|'lbs', yield_shared, remote_entity_id, remote_entity_name the tenant's farming entity operating the field)
