...
export default async function IncomePage({
  ...
  // Chart data: expected vs received per year
  const chartYears = years.map((y) => {
    ...
    const received = t
      ...
      : 0;
    return { year: y, expected, received };
  });
  const maxValue = Math.max(1, ...chartYears.flatMap((c) => [c.expected, c.received]));
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-4 md:p-6">
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-4">
        ...
        <div className="flex items-end gap-6 overflow-x-auto pb-1" style={{ height: 180 }}>
          {chartYears.map((c) => (
            <div key={c.year} className="flex h-full min-w-20 flex-col justify-end">
