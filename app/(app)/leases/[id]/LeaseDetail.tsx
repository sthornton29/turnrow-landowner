...
export default function LeaseDetail({
  ...
  const relevantFarm = useMemo(
    () =>
      leaseFarmScope({
        lands,
        mappings: farmMappings,
        fieldPropertyById: new Map(fields.map((f) => [f.id, f.property_id])),
        tenantEntity,
      }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [lands, farmMappings, fields, tenantEntity]
  );
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4 md:p-6">
      ...
      {needsAssumptions && years.length > 0 ? (
        <section id="assumptions" className="space-y-2 scroll-mt-4">
          <h2 className="text-lg font-semibold text-gray-900">
